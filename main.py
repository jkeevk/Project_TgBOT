...
def start_message(message):
    ...
    bot.send_message(
        message.chat.id,
        "Добро пожаловать, {0.first_name}!\n✨ Я - <b>{1.first_name}</b>, ваш личный помощник для улучшения английского языка!\n\n📚 Я здесь, чтобы сделать ваш процесс обучения интересным и эффективным. Давайте вместе достигать больших результатов!".format(
...
def drop_progress(message):
    ...
    if message.text:
        if message.text.lower() == "да":
            ...
            bot.send_message(
                message.chat.id, "Вернуться в главное меню↩️", reply_markup=markup
            )
...
def start_word_learning_session(message):
    ...
    if message.text == Command.START:
        ...
        if (
            bot_db.count_words_to_learn(user_id) != 0
        ):  # проверка если в таблице words_to_learn ещё остались слова
        ...
        bot.send_message(
            message.chat.id, "Сбросить прогресс и начать заново?", reply_markup=markup
        )
        bot.register_next_step_handler(message, drop_progress)
...
def handle_correct_answer(message, user_id):
    ...
    if bot_db.check_if_learnt(user_id, message.text) == 5:
        bot.send_message(
            message.chat.id, f"🎉Вы выучили слово {message.text}, угадав его 5 раз!🏆"
        )
...
def add_new_word(message, user_id):
    ...
    if len(bot_db.all_words_ids(user_id)) == len(bot_db.all_main_words_ids()):
        ...
        bot.send_message(
            message.chat.id,
            f"🆕Добавлено новое слово для изучения: {new_word}\n"
